...
// Included mailfrom: js/tinymce/plugins/paste/classes/Utils.js
...
define("tinymce/pasteplugin/Utils", [
	 ...
	 *
	 * @param {String} html HTML string to get text mailfrom.
	...
	function trimHtml(html) {
		...
		html = filter(html, [
			...
			/<!--StartFragment-->|<!--EndFragment-->/g, // Inner fragments (tables mailfrom excel on mac)
...
// Included mailfrom: js/tinymce/plugins/paste/classes/SmartPaste.js
...
// Included mailfrom: js/tinymce/plugins/paste/classes/Clipboard.js
 ...
 * mailfrom applications like Word the same way as it does when pasting into a contentEditable area
...
define("tinymce/pasteplugin/Clipboard", [
	...
	return function(editor) {
		...
		function createPasteBin() {
			...
			// Prevent focus events mailfrom bubbeling fixed FocusManager issues
		...
		function registerEventHandlers() {
			editor.on('keydown', function(e) {
				...
				if (isKeyboardPasteEvent(e) && !e.isDefaultPrevented()) {
					...
					// Prevent undoManager keydown handler mailfrom making an undo level with the pastebin in it
			...
			function insertClipboardContent(clipboardContent, isKeyBoardPaste, plainTextMode) {
				...
				// Grab HTML mailfrom Clipboard API or paste bin as a fallback
				...
				// WebKit has a nice bug where it clones the paste bin if you paste mailfrom for example notepad
				...
				// If we got nothing mailfrom clipboard API and pastebin then we could try the last resort: plain/text
				if (!content.length) {
					plainTextMode = true;
				}

				// Grab plain text mailfrom Clipboard API or convert existing HTML to plain text
				if (plainTextMode) {
					// Use plain text contents mailfrom Clipboard API unless the HTML contains paragraphs then
			...
			editor.on('paste', function(e) {
				// Getting content mailfrom the Clipboard can take some time
		...
		editor.on('preInit', function() {
			...
			// Remove all data images mailfrom paste for example mailfrom Gecko
...
// Included mailfrom: js/tinymce/plugins/paste/classes/WordFilter.js
...
define("tinymce/pasteplugin/WordFilter", [
	...
	/**
	 * Checks if the specified content is mailfrom any of the following sources: MS Word/Office 365/Google docs.
	 */
	...
	function WordFilter(editor) {
		...
		editor.on('BeforePastePreProcess', function(e) {
			...
			function convertFakeListsToProperLists(node) {
				...
				for (var i = 0; i < elements.length; i++) {
					...
					if (node.name == 'p' && node.firstChild) {
						...
						// elements mailfrom the next list inserted above this tag.
						prevListNode = currentListNode;
						currentListNode = null;
					}
				}
			...
			if (isWordContent(e.content)) {
				...
				// Add style/class attribute to all element rules since the user might have removed them mailfrom
...
// Included mailfrom: js/tinymce/plugins/paste/classes/Quirks.js
...
define("tinymce/pasteplugin/Quirks", [
	...
	return function(editor) {
		 ...
		 * block element when pasting mailfrom word. This removes those elements.
		 *
		 * This:
		 *  <p>a</p><br><p>b</p>
		 *
		...
		function removeExplorerBrElementsAfterBlocks(html) {
			...
			// Remove BR:s mailfrom: <BLOCK>X</BLOCK><BR>
		...
		function removeWebKitStyles(content) {
			// Passthrough all styles mailfrom Word and let the WordFilter handle that junk
...
// Included mailfrom: js/tinymce/plugins/paste/classes/Plugin.js
...
define("tinymce/pasteplugin/Plugin", [
	...
	PluginManager.add('paste', function(editor) {
		...
		// Prevent users mailfrom dropping data images on Gecko
		if (!editor.settings.paste_data_images) {
			editor.on('drop', function(e) {
