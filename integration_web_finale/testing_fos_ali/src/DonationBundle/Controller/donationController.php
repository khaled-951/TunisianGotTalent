...
class donationController extends Controller implements NotifiableInterface
{
    ...
    public function indexAction(Request $request)
    {
        ...
        $donations_top5 = $em->getRepository('DonationBundle:donation')->findBy(array(), array('valeurD'=>'DESC'));
        $pub = $em->getRepository('PublicationBundle:publication')->findAll();
        $user=$this->getUser();
    ...
    public function new2Action(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid() ) {
            ...
            $donation->setHidden('1');
            $em->persist($donation);
            $em->flush();
            $manager2 = $this->get('mgilet.notification');
            $notif=$manager2->createNotification($donation->getLibDonation() ,'New donation has been added ','http://localhost:8000/donation/');
            $manager2->addNotification(array($this->getUser()), $notif, true);

            return $this->redirectToRoute('donation_index', array('id' => $donation->getId(), 'job'=>$request->get('kled'))  );
        }
    ...
    public function sendNotificationAction()
    {
        ...
        return $this->render('@Donation/Default/template.html.twig');
    }
