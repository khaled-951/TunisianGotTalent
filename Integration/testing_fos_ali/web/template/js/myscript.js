  ...
  $("button").click(
      function(){
          ...
          if($(this).attr('id').substr(0, 18 ) == "submit_new_comment"){
              ...
              if(comment.length >= 2 && comment.length < 65)
              $.ajax({
                  url: "/comment/new/" + id,
                  method: "POST",
...
//<i class="fas fa-eye"></i>Views
    ...
    let check_hashtag = false ;
    let check_content = false ;
    $('#publicationbundle_publication_titrePublication, #edit_publicationbundle_publication_catPublication').change(function(){
        check_title = false ;
        if($(this).val().length >= 5 && $(this).val().length <= 50)
            check_title = true ;
    });

    $('#publicationbundle_publication_catPublication, #edit_publicationbundle_publication_catPublication').change(function(){
        check_hashtag = false ;
        if($(this).val().length >= 5 && $(this).val().length <= 50)
            check_hashtag = true ;
    });

    $('#publicationbundle_publication_src, #edit_publicationbundle_publication_src').change(function(){
        check_content = false ;
        if($(this).val().length >= 5)
            check_content = true ;
    });

    $('#publicationbundle_publication_titrePublication, #publicationbundle_publication_src, #publicationbundle_publication_catPublication').change(function() {
        $('#new_post_1').prop('disabled', true);
        $('#new_post_1').removeClass("active") ;
        let can_post = false;
        if (check_title && check_hashtag &&check_content){
            can_post = true;
            $('#new_post_1').prop('disabled', false);
            $('#new_post_1').addClass("active");
        }
    });
